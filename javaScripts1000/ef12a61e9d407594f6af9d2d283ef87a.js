...
try {
	...
	arr[ preferredDoc.childNodes.length ].nodeType; // NOSONAR
} catch ( e ) {
...
Expr = Sizzle.selectors = {
	...
	pseudos: {
		...
		"selected": function( elem ) {
			...
			if ( elem.parentNode ) {
				elem.parentNode.selectedIndex; // NOSONAR
			}
...
jQuery.fn.extend({
	...
	stop: function( type, clearQueue, gotoEnd ) {
		...
		if ( clearQueue && type !== false ) { // NOSONAR
			this.queue( type || "fx", [] );
		}
...
var propFixObj = {};
propFixObj["fo" +  ["r", "r"][0]] = "htmlFor";
propFixObj["cla" + ["ss", "ss"][0]] = "className";
...
if ( !support.optSelected ) {
	jQuery.propHooks.selected = {
		get: function( elem ) {
			...
			if ( parent ) {
				parent.selectedIndex; // NOSONAR

				// Make sure that it also works with optgroups, see #5701
				if ( parent.parentNode ) {
					parent.parentNode.selectedIndex; // NOSONAR
				}
			}
			return null;
		}
...
(function($){
  $.queryToJson = function(qs, is_decode){
    ...
    for (i = 0, len = q_list.length ; i < len; i++) {
      if (q_list[i]) {
        ...
        if (hash.length < 2) value = key,key = '$nullName';	// NOSONAR // å¦æåªækeyæ²¡ævalue, é£ä¹å°å¨é¨ä¸¢å¥ä¸ä¸ª$nullNameæ°ç»ä¸­	// NOSONAR

        if (!(key in json)) json[key] = is_decode ? decodeURIComponent(value) : value;// å¦æç¼å­å æ ä¸­æ²¡æè¿ä¸ªæ°æ®ï¼åç´æ¥å­å¨              
        else  json[key] = [].concat(json[key], is_decode ? decodeURIComponent(value) : value);// å¦æå æ ä¸­å·²ç»å­å¨è¿ä¸ªæ°æ®ï¼åè½¬æ¢ææ°ç»å­å¨
      }
    }
    return json;
  };
  ...
  $.trans = function(url, options, format){
    ...
    var hooks      = {};  // key : fn , é©å­å½æ°fn()çæ åå½¢åæ¯ååºdataåè¯·æ±data
    var request    = [];  // è¯·æ±éå,[],[{data:{}, process:{success:fn, error:fn, timeout:fn}}] 
    var status     = '';  // ['FREE'|'BUSY']
    var curxhr     = null;// å½åxhrå¯¹è±¡
    var transType  = options['transType'] || 'QUEUE'; //['QUEUE' | 'SEIZE']
    var verifiers  = options['verifiers'] || []; //åæ°æ£æ¥å¨
