   ...
   function Player(pPlayer)
   {
      ...
      function getItem()
      {
         ...
         } else if (pPlayer.options_)
         {
            ...
            ap  = item['data-video-auto-play'];
            cms = cv && cv.reference_id ? cv.reference_id : '';
      ...
      function getDateString(pDate)
      {
         ...
         ds = new Date(pDate);
         ds.setUTCHours(ds.getUTCHours() - 4);
         return ds.toISOString().replace(/-/g, '/').replace(/T/, ' H').replace(/:.*/, '');
      };
      ...
      function Video()
      {
         ...
         try
         {
            vids && vids[0] && (info = JSON.parse(decodeURIComponent(vids[0].getAttribute('data-track-info'))));
         } catch (err)
         {
         }
         ...
         self.shares =
         {
            facebook: { name: 'Video Share Facebook', value: 0 },
            twitter:  { name: 'Video Share Twitter',  value: 0 },
            email:    { name: 'Video Share Email',    value: 0 },
            embed:    { name: 'Video Share Embed',    value: 0 },
            link:     { name: 'Video Share Link',     value: 0 }
         };

         self.iris =
         {
            back:       { name: 'Video Back',        value: 0, notAuto: true },
            forward:    { name: 'Video Forward',     value: 0, notAuto: true },
            mute:       { name: 'Video Mute',        value: 0 },
            unmute:     { name: 'Video Unmute',      value: 0 },
            fullscreen: { name: 'Video Fullscreen',  value: 0 },
            slate:      { name: 'Video Up Next',     value: 0, notAuto: true },
            close:      { name: 'Video Close Fixed', value: 0 },
            play:       { name: 'Video Play',        value: 0, counter: true },
            pause:      { name: 'Video Pause',       value: 0, counter: true },
            'vjs-caps-active':   { name: 'Video Captions Off', value: 0},
            'vjs-caps-inactive': { name: 'Video Captions On',  value: 0},
            'vjs-big-play-button': { name: 'Video Start/Play', value: 0, starter: true},
            'card': { name: 'Video Select', value: 0, notAuto: true}
         };
      ...
      function close(pEvent)
      {
         ...
         if (video && video.started && !(video.file || '').match(/\/solid_black-\d+.mp4/))
         {
            ...
            !video.manual && !notAuto && (ev[ev.length] = 'event37');
            notAuto = false;

            for (i in video.iris)
               if (video.iris[i].value)
               {
                  for (l = 0; l < video.iris[i].value; l++)
                  {
                     list[list.length] = video.iris[i].name;
                     if (!video.iris[i].counter)
                        break;
                  }
                  video.iris[i].notAuto && (notAuto = true);
               }
            ...
            mistats.setOnce('eVar46', video.adId);
            mistats.setOnce('eVar62', video.cc);
            isAMP && (mistats.setOnce('eVar74', 'Vendor: AMP'));
            mistats.setOnce('eVar94', video.index || '0');
            mistats.setOnce('prop28', video.cmsid);
      ...
      function event(pObj)
      {
         ...
         if (!video || index !== curIdx || (video && video.started && video.current > 0 && pObj.type === 'loadstart'))
         {
            close();
            video = new Video();
         }
      ...
      function irisControl(pEvent)
      {
         ...
         to && (type = (to.title || '').toLowerCase().match(/back|forward|mute|unmute|fullscreen|play|pause/) || (to.className || '').match(/vjs-caps-inactive|vjs-caps-active/));
         if (type && video.iris[type[0]])
         {
            video.iris[type[0]].value++;
            video.iris[type[0]].starter && (video.manual = true);
         }
      };
      ...
      function irisUpNextClose(pEvent)
      {
         ...
         while (to && !type)
         {
            type = (to.className || '').match(to.nodeName === 'BUTTON' ? /close|vjs-big-play-button/ : (to.nodeName === 'ARTICLE' && to.parentNode && (to.parentNode.className || '').match(/grid static-zones/) ? /card/ : /slate/));
            to = to.parentNode || null;
         }
      ...
      function initPlayer()
      {
         ...
         icons && (mistats.bind(icons, 'mouseup', irisControl));
         obj && (mistats.bind(obj, 'mouseup', irisUpNextClose));

         obj = mistats.getElementByClassName(/grid static-zones/);
         obj && obj.parentNode && (mistats.bind(obj.parentNode, 'mouseup', irisUpNextClose));
...
mistats.ViewTracker = function ()
{
   ...
   function trackView(pEvent)
   {
      ...
      opl = (Math.round(length / 500) * 500);
      onp = Math.round(length / height);
   ...
   this.getMaxViewed = getMaxViewed;
};
...
mistats.wordCount = function ()
{
   ...
   for (i = 0; !story && i < objs.length; i++)
      ((objs[i].id || '').match(/^content-body/) || (objs[i].className || '').toString().match(/(^|\s)story-body/)) && (story = objs[i]);
...
mistats.lfcInit = function ()
{
   if (!window.localStorage || mistats.isDirectConv)
      return;
   window.localStorage.removeItem('mistats_lfc');
   window.localStorage.setItem('mistats_lfc', JSON.stringify({d: Date.now(), h: {}}));
};
...
mistats.getAuthors = function ()
{
   ...
   {
      body = mistats.getElementByClassName(/(^|\s)story-body/);
      body && (bio = mistats.getElementByClassName('bio', body));
      bio && (objs = mistats.getElementsByClassName('byline', bio));
      if (objs)
         for (i = 0; i < objs.length; i++)
         {
            tmp = clean(objs[i].innerHTML || '');
            tmp && tmp.match(/\S/) && (bylines[bylines.length] = tmp);
         }
      else if (mistats.contentsource && !mistats.contentsource.match(/\||^by\s+|\s+y\s+|^por\s+/i))
         bylines[bylines.length] = 'D=c4';
      else
      {
         objs = document.getElementsByTagName('script');
         for (i = 0; !ld && i < objs.length; i++)
            (objs[i].innerHTML || '').match(/"@type": "NewsArticle"/) && (ld = objs[i]);
         if (ld)
            try
            {
               objs = JSON.parse(ld.innerHTML || '{}') || {};
               objs = objs['author'];
               if (objs && Array.isArray(objs))
                  for (i = 0; i < objs.length; i++)
                     objs[i]['name'] && (bylines[bylines.length] = objs[i]['name']);
            } catch (ldErr)
            {
            }
      }
   }
   mistats.authors = bylines.join(',');
   return mistats.authors;
};
...
mistats.pagelevel === 'Story' && (document.addEventListener('DOMContentLoaded', function ()
{
   ...
   function f()
   {
      ...
      window.EF.getInvisibleImageElement = function (a)
      {
         var b;
...
mistats.doQualtrics = function (pProjId)
{
   var obj;
   ...
   pProjId = pProjId || 'ZN_e4ggyW07IMID52Z';
   ...
   this.go=function(){if(this.check()){var a=document.createElement("script");a.type="text/javascript";a.onload=function(){var pp;pp=setInterval(function(){if(!(window.QSI&&window.QSI.API))return;pp&&(clearInterval(pp));window.QSI.API.load();window.QSI.API.run();},250);setTimeout(function(){pp&&(clearInterval(pp));},30000);};a.src=g;document.body&&document.body.appendChild(a)}};
   ...
   try{(new g(100,"r","QSI_S_" + pProjId, "https://" + pProjId.toLowerCase().replace(/_/, '') + "-mcclatchy.siteintercept.qualtrics.com/WRSiteInterceptEngine/?Q_ZID=" + pProjId)).start()}catch(i){}})();
};
...
(function ()
{
   (mistats.pagelevel || '').match(/Story|Video|Gallery/) && window.addEventListener && (window.addEventListener('load', function ()
   {
      var clk;
      var cto;
      var fcp;
      ...
      fcp = setInterval(function ()
      {
         ...
         if (!(mistats.getElementsByClassName && mistats.adobe))
            return fcpClear();

         !cto && (cto = setTimeout(fcpClear, 10000));
