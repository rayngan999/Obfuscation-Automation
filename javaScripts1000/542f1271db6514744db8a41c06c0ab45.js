    ...
    nextmembers = (function($) {
        ...
        var _updateStatusMenu = function(associate) {
            ...
            if(member_connected) {
                ...
                $('.member-signed').hide();
                $('#comment-post-text').attr('placeholder','Connectez-vous pour Ã©crire ici...');
            ...
            if (true === _hasPopupContext()) {
                ...
                $.ajax({
                    ...
                    if (403 === e.status && 0 < $('form.member-login').length) {
                        ...
                        _popup.content.empty().append('<div class="row"><div class="col-md-12"><div class="bloc cap big-font title">Erreur</div><p>Une erreur est survenue, veuillez nous excuser pour la gÃªne occasionnÃ©e.</p></div><button class="btn btn-primary btn-block connect-popup-close">Fermer</button></div>');
                    }
                }).always(function() {
                    _popup.updateStatus('ready');
                    _popup.content.append('<button title="Close (Esc)" type="button" class="mfp-close">Ã</button>');
                    _init();
                    _updateStatusMenu();
                });
            } else {
        ...
        var _checkFormInput = function(form) {
            ...
            $.each(form.find('div.form-group :input'), function(i, input) {
                ...
                if (input.hasClass('form-required')
                    ...
                    if (confirm && input.val() !== confirm.val()) {
                        group.addClass('has-error');
                        help.append('Valeur diffÃ©rente');
                        has_error = true;
                    }
                }
            });
           ...
           /*if( $(".g-recaptcha").data("callback") == "verifyCaptcha") {
                var response = grecaptcha.getResponse();
                if(response.length == 0) {
                    document.getElementById('g-recaptcha-error').innerHTML = '<span style="color:red;">Captcha invalide !</span>';
                    has_error = true;
                }
           }*/
        ...
        /*
         * Fonction qui nous permet de faire une vÃ©rifaction du nom d'utilisateur
         * @param pseudo
         * @private
         */
        var _onVerificationPseudo = function(pseudo) {
           ...
           // console.log(pseudo);
            ...
            if(typeof strPseudo !='undefined' && strPseudo != '') {
                $.ajax({
                    ...
                    success: function( data ) {
                        if(data == false) {
                            ...
                            msgPseudo.html('<div class="has-error"><div class="form-control help-block">Nom d\'utilisateur dÃ©jÃ  utilisÃ© !</div></div>');
                        } else {
                ...
                usernameRegister.css({'border': '1px solid red'});
                msgPseudo.html('<div class="has-error"><div class="form-control help-block">Nom d\'utilisateur ne peut Ãªtre vide !</div></div>');
            }

        };

        /**
         * Fonction qui nous permet de gÃ©nÃ©rer un speudo
         * @param pseudo
         * @param response
         * @private
         */
            ...
            $.ajax({
                ...
                success: function( data ) {
                    if(data == false) {
                        ...
                        $('#msg-pseudo').html('<div class="has-error"><div class="form-control help-block">Nom d\'utilisateur dÃ©jÃ  utilisÃ© !</div></div>');
        ...
        var _onVerificationEmail = function(email) {
            ...
            if(typeof strEmail !='undefined' && strEmail != '') {
                $.ajax({
                    ...
                    success: function( data ) {
                        ...
                        if(data == false) {
                            ...
                            $('#msg-email').html('<div class="has-error"><div class="form-control help-block">Email dÃ©jÃ  utilisÃ© !</div></div>');
                        } else {
                ...
                $('#register-email').css({'border': '1px solid red'});
                $('#msg-email').html('<div class="has-error"><div class="form-control help-block">Email ne peut Ãªtre vide !</div></div>');
            }
