...
RecentComments.prototype = {
	...
	_createNaviCode: function(navi) {
		...
		if(navi.more) {
			code += '<a "rel=nofollow" class="' + _self.config.olderClass + '" title="Ð±Ð¾Ð»ÐµÐµ ÑÑÐ°ÑÑÐµ">' + _self.param.olderText + '<ins> </ins></a>';
		}
		if(pageNumber >= 2) {
			if(pageNumber > 2) {
				code += '<a "rel=nofollow" class="' + _self.config.newestClass + '" title="ÑÐ°Ð¼ÑÐµ ÑÐ²ÐµÐ¶Ð¸Ðµ"><ins> </ins>' + _self.param.newestText + '</a>';
			}
			code += '<a "rel=nofollow" class="' + _self.config.newerClass + '" title="Ð±Ð¾Ð»ÐµÐµ ÑÐ²ÐµÐ¶Ð¸Ðµ"><ins> </ins>' + _self.param.newerText + '</a>';
		}
	...
	_buildComment: function(item) {
		...
		if(item.timestamp && item.timestamp.length > 0) {
			...
			jQuery('.rc-reviewer',itemInfo).after(timestamp);
		}
	...
	_initLinks: function() {
		...
		list.find('a[href][rel*="external"]').click(function(){
	...
	_enterCommnet: function(ev, args) {
		...
		if(itemToggle.length == 1) {
			...
			itemToggle.attr('title', 'ÑÐ°Ð·Ð²ÐµÑÐ½ÑÑÑ/ÑÐ²ÐµÑÐ½ÑÑÑ');
