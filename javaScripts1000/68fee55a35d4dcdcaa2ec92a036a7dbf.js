...
if (typeof usi_analytics === 'undefined') {
	usi_analytics = {
		...
		usi_app.main = function () {
			...
			usi_app.recommendation_site_cart = "28033";
			...
			usi_app.locale = document.documentElement.lang == "fr-ca" ? "ca_fr" :
				document.documentElement.lang == "en-ca" ? "ca_en" :
				document.documentElement.lang == "en-us" ? "us_en" : "" ;
			usi_app.force_group = usi_commons.gup_or_get_cookie('usi_force_group');

			// Pages
			usi_app.is_confirmation_page = usi_app.url.match("/confirmed") != null;

			// Booleans
			usi_app.is_enabled = usi_commons.gup_or_get_cookie("usi_enable", usi_cookies.expire_time.day, true) != "";
			usi_app.is_suppressed = false;

			if (typeof(usi_app.last_url) === "undefined") {
				usi_app.monitor_for_analytics();
			}
			if (usi_app.url.match("/masks/cart") != null) {
				usi_app.page_data = usi_app.scrape_page();
				usi_app.page_data.forEach(function(product){
					usi_commons.send_prod_rec(usi_app.recommendation_site_cart, product);
				});
				usi_commons.log(usi_app.page_data);
			}

			// Apply coupons
			if (document.querySelector(".cart-coupon-panel button") != null && usi_app.coupon != "") {
		...
		usi_app.delayed_load = function() {
			var control_site_id = '35463';
			usi_split_test.instantiate(control_site_id, usi_app.force_group);
			if (usi_split_test.get_group(control_site_id) == '1') {
				window.usi_force = 1;
				if (usi_date.is_after("2021-07-01T00:00-08:00")) {
					usi_commons.load("MUEJR7z1CkBZl9mRFdU4ynI", "36687", usi_app.locale + "_" + usi_commons.device);
				} else {
					usi_commons.load("iP9oSN8q5qCe2VdAfCl5b1Z", "35167", usi_app.locale + "_" + usi_commons.device);
				}
		...
		usi_app.apply_coupon = function() {
			...
			var coupon_button = document.querySelector(".cart-coupon-panel button");
			if (coupon_button !== null) coupon_button.click();
			var coupon_button = document.querySelector(".cart-coupon-panel button");
			var coupon_input = document.querySelector("#couponEntry");
		...
		usi_app.scrape_page = function() {
			usi_commons.log("usi_app.scrape_page()");
			try {
				var data = usi_app.get_data("products") || [];
				return data.map(function(item){
					var product = {};
					product.link = "https://www.vistaprint.com/masks/all/";
					product.pid = item.product_id;
					product.image = item.design.livePreviewUrl;
					product.name = item.name
					product.price = Number(item.list_price / item.sales_quantity).toFixed(2);
					product.extra = item.variant;
					return product;
				});
			} catch (err) {
				usi_commons.report_error(err);
		...
		usi_app.get_data = function (property) {
		...
		usi_app.monitor_for_analytics = function() {
			try {
				if (typeof(usi_app.last_url) === "undefined" || usi_app.last_url != location.href) {
					...
					usi_analytics.send_page_hit("VIEW", "6907");
				}
		...
		usi_app.ready = function(){
			if (document.querySelector(".preloader-graphic") !== null) {
				return setTimeout(usi_app.ready, 1000);
			}
			usi_app.main();
		}
