...
var ADM_AdsTracking = {
    ...
    check: function (id, val) {
		...
		var chkid=false;
		/*if(id=='__uid'){
			console.log(ADM_AdsTracking.store.get(id),'guid-check');
		}*/
		if(id=='__uid'){
			try{
				var chkid=localStorage.getItem("ads_store>__uid");
				console.log(chkid,'get chkid');
			}catch(e){
				
			}
			if(!chkid){
				var chkid=ADM_AdsTracking.store.get(id);
			}
			if(chkid && chkid!='' && chkid!=val){
				val=chkid;
				try{
					ADM_AdsTracking.store.set(id , chkid);
				}catch(e){}
			}
			else{
				try{
