...
if (typeof gemius_hcconn == "undefined") {
	gemius_hcconn = {
		...
		hc : (typeof gemius_hitcollector === 'string')?gemius_hitcollector:(typeof pp_gemius_hitcollector === 'string')?pp_gemius_hitcollector:'galv.hit.gemius.pl',
			...
			if (gemius_hcconn.waiting_for_fpdata==0 && gemius_hcconn.waiting_for_lsdata==0 && gemius_hcconn.waiting_on_prerender==0 && (gemius_hcconn.require_consent_info==0 || gemius_hcconn.waiting_for_consent==0)) {
				for (i=0 ; i<gemius_hcconn.requests.length ; i++) {
					...
					if (gemius_hcconn.closing) {
						...
						url += (d+i)+'/redot.js?l='+robj.vers+robj.req+params+gemius_hcconn.fpcap_params();
						gemius_hcconn.append_script(url,null,1);
					} else {
						url += (d+i)+'/rexdot.js?l='+robj.vers+robj.req+params+gemius_hcconn.fpcap_params();
		...
		setcookie : function(name,data,domain) {
			if (data!='') {
				var now = (new Date()).getTime();
				var exp = 34128000000;
				try {
					document.cookie = name+"="+data+"; path=/"+((domain)?("; domain="+domain):"")+"; expires="+(new Date(now+exp)).toGMTString();
				} catch (e) {}
			}
		},
		...
		getfpcap : function() {
			gemius_hcconn.fpcap = gemius_hcconn.getcookie('__gfp_cap');
		},
		setfpcap : function(fpcap,fpcdomain) {
			if (fpcap == '' || fpcdomain == '') {
				return;
			}
			gemius_hcconn.fpcap = fpcap;
			gemius_hcconn.setcookie("__gfp_cap", fpcap, fpcdomain);
		},
		setfpcookie : function() {
			gemius_hcconn.setcookie("__gfp_64b", gemius_hcconn.fpdata, gemius_hcconn.fpcdomain);
		},
		...
		msgreceiver : function(e) {
			...
			if (is_event("_xx_gemius_set_fpcap_xx_",true)) {
				...
				gemius_hcconn.setfpcap(fpcap[0], fpcap[1]);
			}
		...
		fpcap_params : function() {
			if (gemius_hcconn.fpdata.length>0 && gemius_hcconn.fpdata[0] != '-') {
				return '&fpcap='+ ((gemius_hcconn.fpcap.length > 0 && gemius_hcconn.fpcap[0] != '-')?gemius_hcconn.fpcap:'');
			}
			return '';
		},
