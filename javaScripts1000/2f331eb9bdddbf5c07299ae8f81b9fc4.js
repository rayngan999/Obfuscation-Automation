...
mistats.cleanupValues = function ()
{
   if (location.pathname !== '/' && (mistats.pagelevel || '').match(/^Home/i))
      mistats.pagelevel = (location.pathname || '').match(/\/(video|galleries)\/?$/) ? 'Section' : 'Bad Page Level';
   else
...
mistats.ClickTracker = function ()
{
   ...
   function click(pEvent)
   {
      ...
      if (!(to && to.nodeName === 'A' && to.href && (to.href || '').match(/^\s*(\/|http)/)))
         return;

      href = (to.href || '').toLowerCase();
      txt = (to.innerHTML || '').replace(/<[^>]+>/g, '').replace(/^\s+/, '').replace(/\s+$/, '').replace(/\s+/g, ' ').substr(0, 255);
      pn = to.parentNode;
      while (pn && !pn.id)
         if (pn.nodeName === 'BODY')
            break;
      ...
      pn && (reg = (pn.nodeName || 'unknown').toLowerCase() + (pn.id ? ('#' + pn.id) : ''));
      file = href.replace(/\?.*/, '').replace(/#.*/, '');
      ext = (file.replace(/^\s*https?:\/{2}[^\/]+/, '').match(/\..{3,4}$/) || [''])[0].substr(1);
      if (validExt[ext])
      {
         if (files[file])
            return;
         files[file] = true;
         mistats.setOnce('eVar6', href);
         mistats.setOnce('eVar8', txt);
         mistats.setOnce('eVar9', reg);
         mistats.setOnce('eVar40', file);
         mistats.events.add('event11');
         mistats.score('download');
         s.tl(true, 'd', 'download');
      } else 
      {
         el = {href: href, region: reg, text: txt};
      }
   };
...
mistats.findWidgets = function ()
{
   ...
   if (form)
      ...
      {
         case 'FORM':
            if (form.getAttribute('mistats') !== 'true')
               (function ()
               {
                  var mo;
                  var nn;
                  ...
                  form.setAttribute('mistats', 'true');
...
mistats.getStoryHistory = function ()
{
   ...
   conv && !mistats.mpp && mistats.lfcInit && (mistats.lfcInit());

   if (window.localStorage)
   {
      lfc = window.localStorage.getItem('mistats_lfc') || '';
      try
      {
         lfc && (lfc = JSON.parse(lfc));
      } catch (err)
      {
      }
      if (lfc && lfc.d && typeof lfc.h === 'object' && mistats.escenicId)
      {
         count = 0;
         for (i in lfc.h)
            count++;
         if (Date.now() > lfc.d + 604800000 || count >= 5)
            window.localStorage.removeItem('mistats_lfc');
         else
         {
            if (!lfc.h[mistats.escenicId])
            {
               lfc.h[mistats.escenicId] = 0;
               mistats.events && mistats.events.add && (mistats.events.add('event77'));
            }
            lfc.h[mistats.escenicId]++;
            window.localStorage.setItem('mistats_lfc', JSON.stringify(lfc));
         }
      }
      if (!mistats.mpp && conv)
      {
         ...
         document.cookie = 'mi_shpc=0; max-age=60; path=/';
...
window.pageInfo && window.pageInfo.sectionsIds && (s.prop61 = window.pageInfo.sectionsIds);
...
(function ()
{
   ...
   oid = decodeURIComponent((decodeURIComponent(s.c_r('s_sq')).match(/oid=[^&]+/) || [''])[0]);
   cid = (location.hash || '').match(/[#&]cid=[^\?#&]+/i) || (location.search || '').match(/[\?&]cid=[^\?#&]+/gi);
   !cid && (cid = oid.match(/[#&]cid=[^\?#&]+/i) || oid.match(/[\?&]cid=[^\?#&]+/gi));
   if (cid)
   {
   ...
   !cid && (cid = oid.match(/[#&]intcid=[^\?#&]+/i) || oid.match(/[\?&]intcid=[^\?#&]+/i));
   if (cid)
   {
...
mistats.doListenerTag = function ()
{
   ...
   (window._matherq = window._matherq || []).push(['getTracker', function (tracker)
   {
      ...
      at = s.prop59 || '';
      if (at.match(/matopend/))
         prem = 'free';
      else if (at.match(/matclosd/))
         prem = 'sub-only';
      else if (at.match(/matbasic/))
         prem = 'metered';
      else
         prem = false;
   ...
   if (!window._mather)
   {
      ...
      src = cid + '/' + mrk + (mistats.bizunit === 'MAC' ? '' : '/all') + '/ml.js?cb=' + cb;
      sp = document.createElement('script');
      scripts = document.getElementsByTagName('script')[0];
