...
function s_doPlugins(s)
{
   ...
   function formatDate(pDate)
   {
      ...
      tmp = (parseInt(dateObj.getMonth()) + 1) + '';
      tmp.length === 1 && (tmp = '0' + tmp);
      date[1] = tmp;
      tmp = parseInt(dateObj.getDate()) + '';
      tmp.length === 1 && (tmp = '0' + tmp);
      date[2] = tmp;
   ...
   s.prop11 = s.prop11.replace(/\|?hasFocus:\w+/g, '');
   s.prop11 = 'hasFocus:' + (document.hasFocus ? (document.hasFocus() ? 'yes' : 'no') : 'unknown') + '|' + s.prop11;
   ...
   if (window.digitalData && window.digitalData.user && window.digitalData.user.adFree)
      s.eVar33 = 'adfree:yes';
   else if (mistats.noSlots)
   {
   ...
   {
      ppn = window.sessionStorage.getItem('mistats_ppn') || '';
      ppn && (s.eVar55 = ppn);
      window.sessionStorage.setItem('mistats_ppn', s.eVar4);
   }
   ...
   if (miSub)
   {
      s.prop9 = miSub.replace(/&e:.+/, '');
      s.eVar97 = {'sub_0': 'Active', 'ex': 'Expired', 'no': 'Registered'}[s.prop9.replace(/mi_sub:s:/, '')] || '';
      s.eVar98 = s.eVar97 === 'Registered' ? 'Registered User' : '';
      exp = parseInt((miSub.match(/&e:\d+/) || [''])[0].substr(3));
      s.eVar99 = exp && !isNaN(exp) ? formatDate(exp / 1000) : '';
   }
   if (mistats.atResponse)
   {
      ...
      if (mistats.atResponse.responseTokens && mistats.atResponse.responseTokens.length)
      {
         ...
         if (obj)
         {
            ...
            s.eVar97 !== 'Active' && s.eVar97 !== 'Registered' && !isNaN(obj['profile.subStatus']) && (s.eVar97 = ['', 'Active', 'Expired', 'Pending', 'Failure Retry', 'Canceled By Autorenew', 'Canceled By Support', 'Canceled By User', 'Registered'][parseInt(obj['profile.subStatus']) || 0] || '');
            s.eVar98 = s.eVar98 || (obj['profile.subTitle'] || '');
            s.eVar99 = s.eVar99 || (obj['profile.subExpires'] ? formatDate(obj['profile.subExpires']) : '');
            s.eVar100 = obj['profile.subRate'] || '';
            s.eVar101 = obj['profile.subDigiScore'] || '';
            s.eVar102 = obj['profile.subPrntScore'] || '';
         }
   ...
   s.eVar104 = (document.cookie || '').length;

   if (window.performance && window.performance.getEntriesByType)
      window.performance.getEntriesByType('mark').find(function (pElem) 
      { 
         var count; 
         count = (((pElem.name || '').match(/dastehdocount\d+/) || [''])[0].match(/\d+/) || [''])[0];
         if (!count) 
            return false; 
         s.eVar84 = count;
         return true;
