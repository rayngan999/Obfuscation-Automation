if (typeof s_account === "undefined") s_account = "xactprod"; // We do this so the implementing site can override if needed
...
s.trackDownloadLinks=false; // This is tracked manually in the plugins section below
s.trackExternalLinks=true
s.trackInlineStats=true
s.linkDownloadFileTypes="exe,zip,wav,mp3,mov,mpg,avi,wmv,doc,pdf,xls,swf,xlsx"; // This is tracked manually in the plugins section below
s.linkInternalFilters="javascript:,xactware.com,cms2,cms1,localhost"
...
s.currentYear=new Date().getUTCFullYear();
s.intUrl="https://testcms.xactware.com/globalassets/us/images/pixel.txt";

/* WARNING: Changing any of the below variables will cause drastic
changes to how your visitor data is collected.  Changes should only be
made when instructed to do so by your account manager.*/
s.visitorNamespace="xactware"
s.trackingServer="stats.xactware.com"
s.trackingServerSecure="sstats.xactware.com"
s.dc=112

/* AJS - Documentation outlines calling s_getLoadTime() in doPlugins section and also near the beginning, outside of any function */
s_getLoadTime()
...
function s_doPlugins(s) {
	...
	// ACM - AppMeasurement version
	s.prop31=s.version;

	// ACM - adding this to get the original referrer and alias passed through server redirects
	if(s.Util.getQueryParam('ref') || s.Util.getQueryParam('url')){
		s.referrer=s.Util.getQueryParam('ref');
		s.pageURL=s.Util.getQueryParam('url');
		s.prop6=s.eVar7="D=g";
	}

	// ACM - Get search query from referrers (Google, Bing, etc)
	var q = s.Util.getQueryParam('q',document.referrer);
	if (q && document.referrer.split('?')[0].indexOf('www.xactware.com') == -1){
		var sfcid = s.Util.getQueryParam('SFCID');
		if (sfcid==="70130000000Klwa" || sfcid==="70130000000XJ9q" || sfcid==="70130000000XBEP"){
			// only capture unique values per search type per session
			q = s.getValOnce(q,'q_p',0);
			if (q) {
				s.eVar2="Paid";
				s.addEvent('event21');
				setSearchTrackingVars(q);
			}
		} else {
			// only capture unique values per search type per session
			q = s.getValOnce(q,'q_n',0);
			if (q) {
				s.eVar2="Natural";
				s.addEvent('event20');
				setSearchTrackingVars(q);
	...
	// ACM - Capture page URL (JIRA MKTG-1656)
	s.eVar23="D=g";
	s.prop29=s.eVar23;

	// ACM - get full referring domains, including subdomains
	s.eVar22 = s.getFullReferringDomains();

	// ACM - Add scOpen event to the first shopping cart visit
	if (window.location.href.indexOf('/store/') > -1){
		var scOpen = s.getValOnce(1,'s_scOpen',0);
		if (scOpen) s.events = s.events ? s.events+",scOpen" : "scOpen";
	}

	// ACM - Calculate time-to-complete events
	if (s.events){
		...
		s.eVar24=s.getTimeToComplete(s.eVar24,'ttc_e30',0);
		s.eVar25=s.getTimeToComplete(s.eVar25,'ttc_e31',0);
		s.eVar26=s.getTimeToComplete(s.eVar26,'ttc_e32',0);
		s.eVar27=s.getTimeToComplete(s.eVar27,'ttc_e33',0);
		s.eVar28=s.getTimeToComplete(s.eVar28,'ttc_e34',0);
		s.eVar29=s.getTimeToComplete(s.eVar29,'ttc_e35',0);
		s.eVar30=s.getTimeToComplete(s.eVar30,'ttc_e36',0);
	}

	// ACM - Day of week/time tracking with -7 GMT offset
	s.eVar39=s.getTimeParting('h','-7'); // Set hour
	s.eVar31=s.getTimeParting('d','-7'); // Set day

	// ACM - Days since last visit
	s.eVar40=s.getDaysSinceLastVisit("s_dslv");

	// ACM - Mobile screen orientation
	s.prop32=screenOrientation();

	// ACM - Segment based on number of visits
	s.eVar49=s.getVisitNum();
	
	// AJS - store page load time in s.prop17
	s.prop17=s.eVar51=s_getLoadTime();
	
	//AJS - Used to determine first page of a visit using both evar43 and sprop 43
	s.eVar43=s.prop43=s.getVisitStart("s_visit");
}
s.doPlugins=s_doPlugins

// ACM - manually track download links
$('a').click(function(e){
	var href = $(this).attr('href').split('=');
	href = href[href.length-1];
	if (href.match('\\.(exe|zip|wav|mp3|mov|mpg|avi|wmv|doc|pdf|xls|swf|xlsx)')){
		setTimeout(function(){
			s.prop12=href;
			s.events="event14";
			s.t();
...
function setSearchTrackingVars(q) {
	s.prop5=q.toLowerCase();
	s.eVar8="D=c5";
	var cd = s.Util.getQueryParam('cd',document.referrer);
	if (cd) {
		s.prop9=cd;
		s.eVar17="D=c9";
		s.addEvent("event1="+s.prop9);
	}
}
...
s.Media.contextDataMapping = {
    "a.media.name": "eVar130,prop54",
    "a.media.segment": "eVar131",
    "a.contentType": "eVar132",
    "a.media.timePlayed": "event106",
    "a.media.view": "event107",
    "a.media.segmentView": "event108",
    "a.media.playerName": "eVar133",
    "a.media.milestones":

        { 25: "event109", 50: "event110", 75: "event111", 100: "event112" }
};
...
/*
* Plugin: ACM - addEvent 0.1 - set s.events or append to existing events
*/
s.addEvent=function(event){if(s.events)s.events+=","+event;else s.events=event};
...
/*
* Plugin: ACM - getTimeToComplete 0.1 - return the time from start to stop (this was rewritten from the Omniture provided plugin that had some bugs for our implementation)
*/
s.getTimeToComplete=function(a,b,c){var d=new Date;if(a==="start"&&document.cookie.indexOf(b)===-1){s.c_w(b,d.getTime(),c)}else if(a==="stop"){var e=s.c_r(b);if(!e)return;s.c_w(b,"",d);var f=(d.getTime()-e)/1e3;var g=86400,h=3600,i=60,j=5,k,l;if(f>g){k=g;l="days"}else if(f>h){k=h;l="hours"}else if(f>i){j=2;k=i;l="minutes"}else{j=.2;k=1;l="seconds"}f=f*j/k;return Math.round(f)/j+" "+l}return}

/*
* Plugin: Mobile screen orientation - http://webanalyticsland.com/sitecatalyst-implementation/capture-mobile-device-screen-orientation-in-sitecatalyst
*/
function screenOrientation(){switch(window.orientation){case 0:case 180:return("Portrait");break;case 90:case -90:return("Landscape");}}

/* Module: Media */
function AppMeasurement_Module_Media(q){var b=this;b.s=q;q=window;q.s_c_in||(q.s_c_il=[],q.s_c_in=0);b._il=q.s_c_il;b._in=q.s_c_in;b._il[b._in]=b;q.s_c_in++;b._c="s_m"; b.list=[];b.open=function(d,c,e,k){var keys=Object.keys(kWidget.userAgentPlayerRules)[0]; keys = 33913162 ? b.playerName = "Xactware TV Player":keys = 37715912? b.playerName = "Xactware Webcasts Player": keys= 39075872 ? b.playerName = "Xactware.com Player" : keys = 39529402 ? b.playerName = "Xactware Basic Player": keys = 39838421 ? 
b.playerName ="Verisk.com v2" : keys = 43059851 ? b.playerName = "Verisk.com No Title AutoPlay": keys = 43581221 ? b.playerName = "Elevate Conference Playlist Player" : keys = 43796442 ? b.playerName = "Xactware.com Player (Poster Look)" : keys = 43839732 ? b.playerName = "XactAnalysis UK Playlist Player" : keys = 43992472 ? b.playerName = "New Player" : b.playerName = keys;var f={},a=new Date,l="",g;c||(c=-1);if(d&&e){b.list||(b.list={});b.list[d]&&b.close(d);k&&k.id&&(l=k.id);if(l)for(g in b.list)!Object.prototype[g]&&b.list[g]&&b.list[g].R==l&&b.close(b.list[g].name);f.name=d;f.length=c;f.offset=0;f.e=0;f.playerName=b.playerName?b.playerName:e;f.R=l;f.C=0;f.a=0;f.timestamp=
...
/*                                                                  
 * Plugin AJS: getLoadTime
 */
function s_getLoadTime(){if(!window.s_loadT){var b=new Date().getTime(),o=window.performance?performance.timing:0,a=o?o.requestStart:window.inHeadTS||0;s_loadT=a?Math.round((b-a)/100):''}return s_loadT}

/*
 *Plugin: Get visit start (AJS)
 */
