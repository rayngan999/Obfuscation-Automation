...
if(!s_account) { var s_account="rea-1form";}
...
s.linkInternalFilters="javascript:,1form.com,m.1form.com"
...
function s_doPlugins(s) {
  ...
  var name_prefix = 'unknown';
  s.events = 'event12';
  if(s.prop24) {
    name_prefix = s.prop24 +':';
  }
  if(typeof omn_obj == 'object') {
    //action
    if(typeof omn_obj.pagename == 'string') {
      name = omn_obj.pagename;
	  
	  
      switch(name) {
        case 'property/details':
          s.pageName = name_prefix + 'property:details:step 1';
        break;
        case 'property/occupancy':
          s.pageName = name_prefix + 'property:occupancy:step 2';
        break;
        case 'extra/questions':
          s.pageName = name_prefix + 'property:agent specific:step 3';
        break;
        case 'property/inspection':
          s.pageName = name_prefix + 'property:inspection:step 4';
        break;
        case 'profile/identification':
          s.pageName = name_prefix + 'profile:identification:step 5';
        break;
        case 'property/declaration':
          s.pageName = name_prefix + 'property:declaration:step 6';
        break;
        case 'property/reviewAndSend':
          s.pageName = name_prefix + 'property:review & send:step 7';
        break;
        case 'submitted application':
          s.pageName = name_prefix + 'property:submitted application:step 8';
          var appid = 'unknown';
          if(typeof omn_obj.appid == 'string') {
            appid = omn_obj.appid;
          }
          s.events = 'event12,event1:'+ appid 
          s.prop26 = s.eVar26 = appid;
        break;
        }
    }
  }
  //console.log(s.events);
    ...
    if(!s.campaign) s.campaign=s.getQueryParam('rsfdetail');
  if(!s.campaign) s.campaign=s.getQueryParam('rsf');

    /* Set Traffic Source Detailed Variable */
  if(!s.prop30) s.prop30=s.getQueryParam('rsfdetail');
...
if (s.events) {
  ...
  if(!s.eVar13) s.eVar13=s.getQueryParam('pid');
  if(!s.eVar13) s.eVar13=s.getQueryParam('cid');
  ...
  if(!s.prop1) { 
      if(typeof REA == 'object') {
        s.prop1 = REA.omniture.getProp1Value();
  ...
  /* Vendor lead submit */
  // updated by digital balance to counter the resetting of s.events to just event12 on line 247 of this file
/*
  if ((s.pageName) && (s.pageName == "rea:buy:leads:rea-form-submit")) {
	  s.events = s.events + ',event91';
  }
*/
  // address mobile version of the submit page
/*
  if ((s.pageName) && (s.pageName == "reamob:buy:leads:agency-form-submit")) {
	  s.events = s.events + ',event91';
  }
*/
  
  if ((document.location.href.indexOf("https://property.value.realestate.com.au/view/report/") > -1) && (document.referrer.indexOf("https://property.value.realestate.com.au/") > -1)) {
	  s.events = 'event12,event91';
    // serialise the event91 against the report id if it's present
    var s_report_id_array = document.location.pathname.split("/");
    if ((s_report_id_array) && (s_report_id_array[3] != "") && (s_report_id_array[3] != undefined)) {
        s.events = s.events + ":" + s_report_id_array[3];
    }
    s.pageName = 'rea:buy:leads:rea-form-submit';
	  s.channel = 'buy';
	  s.hier1 = 'rea|buy|leads';
	  s.server = 'property.value.realestate.com.au';
	  s.prop11 = 'rea:buy:leads';
	  s.prop12 = 'rea:buy:leads:rea';
	  s.prop18 = 'rea-form-submit';
	  s.prop24 = 'rea';
    s.prop74 = s.eVar74 = s.pageName + "|" + document.location.href;
  }
  ...
  if (typeof s.pageName!="undefined") {s.pageName=constrain(s.pageName.toLowerCase(),'255');}
  if (typeof s.hier1!="undefined") {s.hier1=constrain(s.hier1.toLowerCase(),'255');}
  if (typeof s.eVar13!="undefined") {s.eVar13=constrain(s.eVar13.toLowerCase(),'255');}
  if (typeof s.eVar25!="undefined") {s.eVar25=constrain(s.eVar25.toLowerCase(),'255');}
  if (typeof s.eVar27!="undefined") {s.eVar27=constrain(s.eVar27.toLowerCase(),'255');}
  if (typeof s.eVar28!="undefined") {s.eVar28=constrain(s.eVar28.toLowerCase(),'255'); s.prop28=s.eVar28;}
  if (typeof s.channel!="undefined") { s.channel=constrain(s.channel.toLowerCase(),'255'); if(s.channel&&!s.eVar16) s.eVar16=s.channel;}
  
  /* DTM debug variable */
  s.prop71 = s.eVar71 = "DTM";
...
s.trackingServer = "reagroup.122.2o7.net"
s.trackingServerSecure = "reagroup.122.2o7.net"
